...
namespace MapReduceSamples.Utils
{
    public class BloomFilter<T>
    {
        ...
        {
            var size = Convert.ToInt32(Math.Ceiling(_hashBits.Length/8.0));
            var buffer = new byte[size];
            _hashBits.CopyTo(buffer, 0);
            var base64 = Convert.ToBase64String(buffer);

            File.WriteAllText(filename, String.Format("{0};{1};{2}", _hashFunctionCount, _hashBits.Length, base64));
        }

        public static BloomFilter<T> Load(string filename)
        {
            return Load(filename, null);
        }

        public static BloomFilter<T> Load(string filename, HashFunction hashFunction)
        {
            var fileText = File.ReadAllText(filename);
            var fileElements = fileText.Split(";".ToCharArray(), 3);

            var k = Convert.ToInt32(fileElements[0]);
            var m = Convert.ToInt32(fileElements[1]);
            var data = Convert.FromBase64String(fileElements[2]);

            var filter = new BloomFilter<T>(1, 0.01f, hashFunction, m, k)
            {
                _hashBits = new BitArray(data),
            };
            filter._hashBits.Length = m;
